...
if __name__ == '__main__':
    ...
    url = 'https://www.flinnsci.com/products/'
        ...
        if inner_req.find('a', class_='b-categories__category__link'):
            ...
            for category in inner_category:
                ...
                if category_req.find('div', id='FilteredListList'):
                    ...
                    page_nav_number = category_req.find('h3', class_='hidden-lg hidden-xs b-filtered-list__nav-heading__current-category').text.split(
                        '(', 1)[-1].replace(')', '').strip()
                            ...
                            if not product_request.find('ul', class_='product-page__info--options list'):
                                ...
                                for single_product in sub_product:
                                    ...
                                    try:
                                        if single_product.find('span', class_='product-page__price'):
                                            product_price = strip_it(
                                                single_product.find('span', class_='product-page__price').text.strip())
                                        elif single_product.find('div', class_='product-page__price'):
                                            product_price = strip_it(
                                                single_product.find('div', class_='product-page__price').text.strip())
                                        else:
                                            product_price = ''
                                    except:
                                        product_price = ''
                                    ...
                                    if product_url in read_log_file():
                                        continue
                                    print('current datetime------>', datetime.now())
                                    dictionary_1 = get_dictionary(product_ids=product_id, product_names=product_name, product_quantities=product_quantity, product_prices=product_price, product_urls=product_url)
                            ...
                            for single_contents in content_json:
                                ...
                                if not product_request.find('ul', class_='product-page__info--options list'):
                                    ...
                                    if product_url in read_log_file():
                                        continue
                                    print('current datetime------>', datetime.now())
                                    dictionary_2 = get_dictionary(product_ids=product_id, product_names=product_name, product_quantities=product_quantity, product_prices=product_price, product_urls=product_url)
                                    ...
                                    sub_product = product_request.find('ul',
                                                                       class_='product-page__info--options list').find_all(
                                        'li')
                                    for single_product in sub_product:
                                        product_url = f'{base_url}{single_product.find('a', class_='option-link col-xs-12 col-sm-6')['href']}'
                                        '''PRODUCT NAME AND PRODUCT ID'''
                                        ...
                                        try:
                                            if single_product.find('span', class_='product-page__price'):
                                                product_price = strip_it(single_product.find('span',
                                                                                             class_='product-page__price').text.strip())
                                            elif single_product.find('div', class_='product-page__price'):
                                                product_price = strip_it(single_product.find('div',
                                                                                             class_='product-page__price').text.strip())
                                            else:
                                                product_price = ''
                                        except:
                                        ...
                                        if product_url in read_log_file():
                                            continue
                                        print('current datetime------>', datetime.now())
                                        dictionary_3 = get_dictionary(product_ids=product_id, product_names=product_name, product_quantities=product_quantity, product_prices=product_price, product_urls=product_url)
                ...
                for single_contents in content_json:
                    ...
                    if not product_request.find('ul', class_='product-page__info--options list'):
                        ...
                        if product_url in read_log_file():
                            continue
                        print('current datetime------>', datetime.now())
                        dictionary_4 = get_dictionary(product_ids=product_id, product_names=product_name, product_quantities=product_quantity, product_prices=product_price, product_urls=product_url)
                        ...
                        for single_product in sub_product:
                            ...
                            try:
                                if single_product.find('span', class_='product-page__price'):
                                    ...
                                    product_price = strip_it(single_product.find('div', class_='product-page__price').text.strip())
                                else:
                                    product_price = ''
                            except:
                                product_price = ''
                            ...
                            if product_url in read_log_file():
                                continue
                            print('current datetime------>', datetime.now())
                            dictionary_5 = get_dictionary(product_ids=product_id, product_names=product_name, product_quantities=product_quantity, product_prices=product_price, product_urls=product_url)
